...
public interface IOrderDao {
    ...
    @Update("insert into orders (o_date, o_payment, o_u_id, o_price, o_status, o_content) values(#{oDate},#{oPayment},#{oUId},#{oPrice},#{oStatus},#{oContent})")
    ...
    @Select("select *from orders where o_u_id = #{oUId}")
    @ResultMap("orderMap")
    List<Orders> findByUId(Integer oUId);
    @Update("delete from orders where o_id = #{oId}")
    void delOrders(Integer oId);
    @Select("select *from orders where o_date >= #{begin} and o_date <= #{end}")
    @ResultMap("orderMap")
